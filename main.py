...
class Repeater(DBSide, VKSide, nextcord.Client):
    def __init__(self):
        ...
        self.description = ['Это Repeater!',
                            ...
                            '/add <url VK сообщества> <пинг=да/нет/ID роли, '
                            'которую надо пинговать при отправке нового поста> - |ТРЕБУЮТСЯ ПРАВА АДМИНИСТРАТОРА| '
                            'добавляет в подписки канала переданное сообщество. Параметр "пинг" '
                            'настраивает пинг при отправки постов в этот канал (по умолчанию никто не пингуется).'
                            ' "пинг=да" сделает так, что каждый пост будет '
                            'сопровождаться "@everyone" в конце, если же Вы укажете ID какой-то другой роли, '
                            'то пинговаться будет она. Работать в ветках данная команда не будет.\n',
